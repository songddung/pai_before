...
import { MOCK_CONFIG, quizMockApi } from '../../../shared/api/mock';
...
export const quizApi = {
  ...
  async createQuiz(data: CreateQuizRequest) {
    if (MOCK_CONFIG.USE_MOCK_API) {
      return quizMockApi.createQuiz(data);
    }
  ...
  async updateQuiz(quizId: string, data: Partial<CreateQuizRequest>) {
    if (MOCK_CONFIG.USE_MOCK_API) {
      return quizMockApi.updateQuiz(quizId, data);
    }
  ...
  async deleteQuiz(quizId: string) {
    if (MOCK_CONFIG.USE_MOCK_API) {
      return quizMockApi.deleteQuiz(quizId);
    }
  ...
  async getAvailableQuizzes(): Promise<any[]> {
    if (MOCK_CONFIG.USE_MOCK_API) {
      return quizMockApi.getAvailableQuizzes();
    }
  ...
  async submitQuiz(quizId: string, data: SubmitQuizRequest): Promise<QuizSubmitResult> {
    if (MOCK_CONFIG.USE_MOCK_API) {
      return quizMockApi.submitQuiz(quizId, data);
    }
  ...
  async getChildrenQuizResults(): Promise<QuizResult[]> {
    if (MOCK_CONFIG.USE_MOCK_API) {
      return quizMockApi.getChildrenQuizResults();
    }
  ...
  async giveReward(quizResultId: string) {
    ...
    if (MOCK_CONFIG.USE_MOCK_API) {
      return quizMockApi.giveReward(quizResultId);
    }
